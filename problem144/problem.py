...
def reflect(beamSlope, beamDir, wallSlope):
	...
	assert type(beamSlope) is float, "beamSlope must be a float."
	assert type(beamDir) is int and beamDir in [-1.0, 0.0, 1.0], "beamDir must be in [-1, 0, 1]."
	assert type(wallSlope) is float, "wallSlope must be a float."
	if beamSlope in [float("inf"), float("-inf")]:
		assert beamDir == 0.0, "If the beam slope is infinite, the direction should be zero."
	else:
		assert beamDir in [1.0, -1.0], "If the beam slope is non-infinate, there must be a " + \
...
while newSpot[0] < -0.01 and newSpot[0] > 0.01 and newSpot[1] < 9:
	wallSlope = 0
	beamSlope = 0
	beamDir = 0
